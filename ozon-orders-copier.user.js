    ...
    function parseOrdersFromStateJSON() {
        try {
            ...
            const orders = ordersArr.map((order, oi) => {
                ...
                const pickupPoint = titleText.replace(/^Пункт\s+Ozon[:\s]*/i, '').trim();
        ...
        try {
            ...
            try {
                ...
                const _addrHashSelectors = [
                    '.b35_5_2-b4.tsBody400Small',   // актуальная разметка (2026)
                    '.b35_5_1-b4.tsBody400Small',   // предыдущая разметка
                ];
                let addrEl = null;
                ...
                if (!addrEl) {
                    ...
                    for (const el of _candidates) {
                        const t = (el.textContent || '').trim();
                        if (/^Пункт\s+Ozon,\s*.+/i.test(t)) { addrEl = el; break; }
                    }
                }
                ...
                Diagnostics.logParseResult(orderNumber, 'detailsAddress',
                    'каскад: .b35_5_2-b4 → .b35_5_1-b4 → [class*=b35] span{^Пункт Ozon,} → .tsCompactControl500Medium',
                    detailsAddress);
